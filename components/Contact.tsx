...
const Contact = () => {
  ...
  const x = useSpring(useTransform(mouseX, [-100, 100], [-300, 300]), springConfig)
  const y = useSpring(useTransform(mouseY, [-100, 100], [-300, 300]), springConfig)
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
            ...
            >
              ...
              {/* Services Selection */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-4">I'm interested in...</label>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {services.map((service) => (
                    <motion.button
                      ...
                      className={`py-2 px-3 rounded-lg text-sm font-medium transition-all duration-200 ${selectedServices.includes(service)
              ...
              {/* Budget Selection */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-4">Project Budget (USD)</label>
                <div className="flex space-x-3">
                  {budgets.map((budgetOption) => (
                    <motion.div
                      ...
                      onMouseMove={budgetOption === 'Free' ? handleMouseMove : undefined}
                      onClick={budgetOption === 'Free' ? handleFreeChipClick : undefined}
                      className={`relative ${budgetOption === 'Free' && 'border z-[9999]'}`}
                    >
                      <button
                        type="button"
                        disabled={budgetOption === 'Free'}
                        onClick={() => handleBudgetSelect(budgetOption)}
                        className={`py-2 px-4 rounded-lg text-sm font-medium transition-all duration-200 ${budget === budgetOption
                          ? 'bg-gray-200 border-2 border-gray-900'
                          : 'text-gray-900 hover:bg-gray-100'
                          } ${budgetOption === 'Free' ? 'cursor-not-allowed opacity-60' : ''
                          }`}
                      >
                        {budgetOption}
                      </button>
              ...
              {/* Project Description */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-4">Tell us about your project.</label>
              ...
              <div className="space-y-6">
                ...
                <motion.button
                  ...
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="w-full bg-gray-900 hover:bg-gray-800 text-white font-medium py-4 px-8 rounded-xl transition-all duration-300"
                >
                  Submit
                </motion.button>
              </div>
